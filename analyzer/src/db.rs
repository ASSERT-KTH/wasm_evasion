use core::slice;
use std::{fmt::Debug, borrow::Borrow, cell::RefCell, io::{BufReader, Read}};
...
impl<'a> DB<'a> {
    ...
    pub fn set<T>(&self, k: &AsRef<[u8]>, v: T) -> AResult<bool> 
        where T: Serialize
    {
        self.db.insert(k, serde_json::to_string_pretty(&v)?.as_bytes())?;
        self.db.flush()?;
        Ok(true)
    }
        ...
        K: AsRef<[u8]> + Debug
    {
        ...
        match kvalue {
            None => {
                return Err(CliError::KeyNotFound(format!("{:?}", k)))
            }
            Some(v) => {
                return Ok(v.to_vec())
            }
