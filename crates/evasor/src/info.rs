...
macro_rules! get_info {
    ($mutation: expr, $config: ident, $state: ident, $meta: ident, $prettyname: literal, $description: literal, $reduce: literal, $tpe: expr, $affects_execution: literal, $rs: ident, $seed: ident, $sample_ratio: ident, $stopsignal: ident) => {
        { if $config.is_some() && $mutation.can_mutate(&$config) {
            ...
            $rs.push(
                (MutationInfo{ class_name: format!("{} sample {} timeout {:?} seed  {}",stringify!($mutation), $sample_ratio, is_sample_due_to_timeout, $seed), pretty_name:$prettyname.to_string(), desccription: $description.to_string(), map: (0, "".into()), can_reduce: $reduce, tpe: $tpe.get_val(), affects_execution:$affects_execution, generic_map: None, sampled: is_sample_due_to_timeout }, idxsmap)
            );
        }
    }
    }
