...
use crate::{
    ...
    info::InfoExtractor,
    meta::{self, Meta},
    subcommands::export::create_chunk,
    State, NO_WORKERS,
};

pub fn get_single_wasm_info(
    f: &PathBuf,
    state: Arc<State>,
    sample: u32,
    seed: u64,
    ...
    match &buf {
        b"\0asm" => {
            ...
            let info = std::panic::catch_unwind(move || InfoExtractor::get_info(&cp, &mut meta));
                ...
                Ok((mut info, mut mutations)) => {
                    ...
                    if let Some(client) = &state.dbclient {
                        ...
                        match dbclient.set(&info.id.clone(), info) {
                            Ok(_) => {}
                            Err(e) => match e {
                                CliError::ThreadTimeout => return Err(CliError::ThreadTimeout),
                                _ => {
                                    log::error!("{:?}", e)
                                }
                            },
    ...
    loop {
        ...
        if let Some(f) = f {
            ...
            loop {
                ...
                let s = chunk.lock().unwrap().len();
                log::debug!(
                    "Restarting thread. Worklist size {}/{}",
                    s + 1, /* the one already working */
                    total
                );
                let time = time::Instant::now();
                let th = spawn(move || {
                    get_single_wasm_info(&fcp.clone(), movecp.clone(), sample, seed, signalcp)
                });
                    ...
                    Err(e) => {
                        match e {
                            CliError::ThreadTimeout => {
                                ...
                                if sample > 1024 {
                                    log::error!(
                                        "The binary {} cannot be processed",
                                        fcp2.clone().display()
                                    );
                                    break;
                                }
...
pub fn get_only_wasm(
    ...
    let filtered = files
        ...
        .filter(move |&f| {
            ...
            ccp.dbclient
                .as_ref()
                .unwrap()
                .get::<String, Meta>(&f.file_name().unwrap().to_str().unwrap().to_string())
                .is_err()
        })
        .map(|f| f.clone())
        .collect::<Vec<_>>();
    ...
    state
        .process
        .store((files.len() - filtered.len()) as u32, Ordering::SeqCst);
